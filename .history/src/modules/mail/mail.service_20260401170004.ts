...
export class MailService {
  ...
  constructor(private readonly configService: ConfigService) {
    this.transporter = createTransport({
      ...
      secure: Number(this.configService.getOrThrow<number>('SMTP_PORT')) === 465,
      auth: {
...
this.configService.getOrThrow<string>('SMTP_PASS'),
  ...
  async sendWelcomeEmail(email: string, name?: string): Promise<void> {
    try {
      await this.transporter.sendMail({
        from: this.configService.getOrThrow<string>('EMAIL_FROM'),
        to: email,
        subject: 'Welcome to AI Notes Nest',
        html: `<p>Hello ${name ?? 'there'}, welcome to AI Notes Nest.</p>`,
      });
    } catch (error) {
      this.logger.warn(`Failed to send welcome email to ${email}: ${String(error)}`);
    }
  }
}
